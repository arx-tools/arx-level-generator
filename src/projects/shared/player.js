...
// component: player
ON GAME_READY {
  IF (§fighting_amb == 0) AMBIANCE -v 0 ambient_fight
 ACCEPT
 }
 ...
 SET #DEBUG 0
 SET §cheat 0
 //TIMERf 0 2 HEROSAY ^#PLAYER_LIFE
  //SET #CRASHTEST 1
  SET #SHUT_UP 0
 ...
 // LOADANIM FIGHT_WALK_MINISTEP        "human_fight_walk_small_steps"
 ...
 // 7= castle free -> loofs for ultimate weapon
  ...
  SET #CINE_SYLIB 0
  SET #SEEN_BANK_KEY 0
  //SET #shany_kindapped 0    //Not used anymore...
 ...
 // CRITICAL {
  //EAK [player_attack_1handed_weap] NOP
  //CEPT
 ///
 ON OUCH {
  HEROSAY -d ^#PARAM1
  IF (^#PARAM1 > 20) SPEAK [Player_ouch_strong] NOP
  ELSE IF (^#PARAM1 > 8) SPEAK [Player_ouch_medium] NOP
  ELSE IF (^#PARAM1 > 2) SPEAK [Player_ouch] NOP
  IF (§fightnow == 0) {
   SET §fightnow 1
   TIMERzgeg 1 5 SET §fightnow 0
  }
  IF (§fighting_amb == 1) ACCEPT
  SET §fighting_amb 1
 // TIMERxx -m 11 150 GOTO AMBIANCE_
 ...
 >>AMBIANCE_FIGHT_END
 //TIMERambientdown -m 11 150
  IF (§fightnow == 0) GOTO AMBIANCE_DOWN
 ...
 >>AMBIANCE_UP
 INC §vol 10
 IF (§vol > 100) SET §vol 100
 ACCEPT
 
 >>AMBIANCE_DOWN
 HEROSAY -d "STOP"
 DEC §vol 10
 IF (§vol < 0) SET §vol 0 SET §fighting_amb 0
 AMBIANCE -v 0 ambient_fight
 ACCEPT
